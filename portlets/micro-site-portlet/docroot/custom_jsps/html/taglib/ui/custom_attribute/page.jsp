...
String typeTag = StringPool.BLANK;
if(className.split(StringPool.COLON).length > 1){
	String nameType[] = className.split(StringPool.COLON);
	className = nameType[0];
	typeTag = nameType[1];
}
	...
	<c:if test="<%= !propertyHidden && ExpandoColumnPermissionUtil.contains(permissionChecker, company.getCompanyId(), className, ExpandoTableConstants.DEFAULT_TABLE_NAME, name, ActionKeys.VIEW) %>">
		...
		<c:choose>
			<c:when test="<%= editable && ExpandoColumnPermissionUtil.contains(permissionChecker, company.getCompanyId(), className, ExpandoTableConstants.DEFAULT_TABLE_NAME, name, ActionKeys.UPDATE) %>">
				...
				inlineLabel='<%= Boolean.toString("checkboxes".equals(typeTag)) %>'>
					...
					<c:choose>
						<c:when test="<%= type == ExpandoColumnConstants.BOOLEAN %>">
							...
							<%-- START RIVET LOGIC CUSTOMIZATION --%>
							<c:choose>
								<c:when  test='<%= "checkboxes".equals(typeTag) %>'>
									<input type="checkbox" id="<%= randomNamespace + escapedName %>" 
									value="<%=curValue ? "1" : "0" %>" 
									 name="<portlet:namespace />ExpandoAttribute--<%= escapedName %>--"
									 style="display:inline;float:left;margin-right:8px">
									 <aui:script use="aui-node">
									 	var checkBox = A.one('#<%= randomNamespace + escapedName %>');
									 	if(<%=curValue%>){
